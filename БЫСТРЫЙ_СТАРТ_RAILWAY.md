...
✅ Миграции БД перенесены в runtime (выполняются при запуске, а не при сборке)  
✅ Создан `railway_start.sh` - скрипт запуска с миграциями  
✅ Улучшен healthcheck - теперь не падает без Redis/Celery  
✅ Обновлен `railway.toml` для использования нового скрипта запуска  
